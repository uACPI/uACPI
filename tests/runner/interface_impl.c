...
void uacpi_kernel_unmap(void *addr, uacpi_size len)
{
    ...
    if (!mapping) {
        printf(
            "WARN: cannot identify mapping virt=%p phys=0x%016" PRIX64 " with s"
...
void uacpi_kernel_free(void *mem, uacpi_size size_hint)
{
    ...
    if (!allocation) {
        fprintf(stderr, "unable to find heap allocation %p\n", mem);
        abort();
    }
...
{
    microsecond_sleep(usec);
}

void uacpi_kernel_sleep(uacpi_u64 msec)
{
    microsecond_sleep(msec * 1000);
}
